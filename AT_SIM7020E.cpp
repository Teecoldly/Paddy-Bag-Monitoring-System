...
void AT_SIM7020E::setupModule(String address, String port) {
  ...
  if (tcp) opensoketcp(address, port);
  if (udp) opensokeudp(address, port);
  if (tcp == false && udp == false) {
    attachNetwork(address, port);
  }
}
void AT_SIM7020E::setupModule_fast(){
  previous_check = millis();
...
void AT_SIM7020E::opensoketcp(String address, String port) {
  if (attachNetwork(address, port)) {
    if (!createTcpSocker(address, port)) {
      Serial.println(">> Cannot create socket");
    }
    Serial.println(F("OK"));
    Serial.println(F("---------- Connected tcp----------"));
  }
  else {
...
void AT_SIM7020E::opensokeudp(String address, String port) {
  if (attachNetwork(address, port)) {
    if (!createUDPSocket(address, port)) {
      Serial.println(">> Cannot create socket udp");
    }
    Serial.println(F("OK"));
    Serial.println(F("---------- Connected udp----------"));
  }
  else {
    Serial.println(F("FAILED"));
    Serial.println(F("-------- Disconnected ---------"));
...
bool AT_SIM7020E::createTcpSocker(String address, String port) {
  bool status = false;
  _Serial->println(F("AT+CSOC=1,1,1"));
  delay(500);
  while (1) {
    if (_Serial->available()) {
...
// Close a UDP/TCP socket 0
bool AT_SIM7020E::closeSocket() {
...
// http //
void AT_SIM7020E::http_post(String httpip , String page, String data_send) {

  _Serial->print(F("AT+CHTTPCREATE="));
  _Serial->print("\"" + httpip + "\"");
  _Serial->println();
  while (1) {
    if (_Serial->available()) {
      ...
      else if (data_input.indexOf(F("+CHTTPCREATE: 1")) != -1) {
  ...
  ;
  delay(1000);
  _Serial->println(F("AT+CHTTPCON=0"));
  _Serial_print(F("AT+CHTTPSEND=0,0,"));//http command
  _Serial_print("\"" + page + "?"); //page send
    _Serial_print(data_send+"\"");
  _Serial_println();
   _Serial->println(F("AT+CHTTPDISCON=0"));
  http_close();
    
  //   _Serial_print
  //   String msg =String(data_send,HEX);
  //   _Serial_println( msg);//content_string
  delay(5000);
  
}

void AT_SIM7020E::http_close() {
